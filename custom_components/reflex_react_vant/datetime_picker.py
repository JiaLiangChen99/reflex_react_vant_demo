...
class DatetimePicker(ReactVantBase):
    ...
    # 事件处理
    on_change: rx.event.EventChain = None
    on_confirm: rx.event.EventChain = None
    on_cancel: rx.event.EventChain = None
    ...
    def _get_props(self):
        ...
        camel_case_props["minDate"] = Var.create("new Date(new Date().getFullYear() - 10, 0, 1)", _var_is_local=True)
        camel_case_props["maxDate"] = Var.create("new Date(new Date().getFullYear() + 10, 11, 31)", _var_is_local=True)
